...
void spi_write(u16 x) {
    ...
    u32 y = (1 << 16) | x; // {done, x}
    while(idx < 4) {
        while(!(XSpi_GetStatusReg(&SpiInstance) & XSP_SR_TX_FULL_MASK) && idx < 2) {
...
void spi_listen() {
    ...
    if(op[0] == 0) write_tile((op[1] << 8) | op[2], op[3], op[4]), spi_write(y);
    if(op[0] == 1) write_weight((op[1] << 8) | op[2], op[3], op[4], op[5]), spi_write(y);
    if(op[0] == 2) write_network_input((op[1] << 8) | op[2], op[3]), spi_write(y);
    ...
    if(op[0] == 6) spi_write(read_input((op[1] << 8) | op[2]));
    if(op[0] == 7) spi_write(read_spk_out((op[1] << 8) | op[2]));
    if(op[0] == 8) timestep(), spi_write(y);
    if(op[0] == 9) reset_model(), spi_write(y);
}