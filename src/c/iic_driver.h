...
void setup_spi() {
    ...
    XIIC_Config *cfg_pointer = XIIC_LookupConfig(IIC_BASE_ADDR);
    if(cfg_pointer == NULL) xil_printf("lookupconfig failed\n\r");
    status = XIIC_CfgInitialize(&SpiInstance, cfg_pointer, cfg_pointer->BaseAddress);
    if(status != XST_SUCCESS) xil_printf("cfginitialize failed\n\r");
    status = XIIC_SetOptions(&SpiInstance, XSP_CLK_PHASE_1_OPTION | XSP_CLK_ACTIVE_LOW_OPTION);    
    if(status != XST_SUCCESS) xil_printf("setoptions failed\n\r");
    
    XIIC_Start(&SpiInstance);
    XIIC_IntrGlobalDisable(&SpiInstance);
}

void iic_write(u16 x) {
    u8 idx;
    idx = 0;
    u32 y = x; // {done, x}
    while(idx < 4) {
        while(!(XIIC_GetStatusReg(&SpiInstance) & XSP_SR_TX_FULL_MASK) && idx < 4) {
            u8 val = (y % (1 << (idx*8+8))) >> (idx*8);
            //xil_printf("write %d\n", val);
            XIIC_WriteReg(SPI_BASE_ADDR, XSP_DTR_OFFSET, val); 
            idx++;
        }
    }
}
...
void spi_listen() {
    ...
    while(idx < 8) {
        while(!(XIIC_GetStatusReg(&SpiInstance) & XSP_SR_RX_EMPTY_MASK) && idx < 8) {
            u8 arg = XIIC_ReadReg(SPI_BASE_ADDR, XSP_DRR_OFFSET);
            //xil_printf("%d %d\n\r", idx, arg);
