...
for i in range(0, MAX_TILES, BRAM_DATA_WIDTH//(2*TILE_IDX_WIDTH)):
	s = ""
	for j in range(i, i+BRAM_DATA_WIDTH//(2*TILE_IDX_WIDTH)):
		s += bin_(tile_idx[i][0], TILE_IDX_WIDTH) + bin_(tile_idx[i][1], TILE_IDX_WIDTH)
	arr[i// (BRAM_DATA_WIDTH//(2*TILE_IDX_WIDTH))] = s
